...
async function generateMovementFlowPDF({ day } = {}) {
  try {
    ...
    const avgBuildingsPerPerson = avgBuildingsPerPersonRes.rows[0]
      ...
      : 0;
    // Round to nearest integer for display (no decimals)
    const avgBuildingsPerPersonRounded = Math.round(avgBuildingsPerPerson);

    // charts data
    const slotLabels = entryExitSlots.map(r => r.slot); // X
    const slotEntries = entryExitSlots.map(r => safeNum(r.entries)); //y
    const slotExits = entryExitSlots.map(r => safeNum(r.exits)); //y
    ...
    try {
      ...
      if (avgBuildingsPerPersonRounded > 0) {
        avgBuffer = await chartJSNodeCanvas.renderToBuffer({
          ...
          data: {
            ...
            datasets: [{
              label: "Avg Buildings",
              data: [avgBuildingsPerPersonRounded],
  ...
  drawKPI(510, 100, "Avg Buildings / Person", avgBuildingsPerPersonRounded);
