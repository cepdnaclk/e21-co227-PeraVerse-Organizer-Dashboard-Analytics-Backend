...
// Prefer db2 (cloud/local via env). Fallback to db1 if db2 unavailable.
let pool;
try {
  pool = require("../utils/db2.js");
} catch (e) {
  try {
    pool = require("../utils/db1.js");
    console.warn("db2.js not found, fell back to db1.js");
  } catch (e2) {
    console.error("No database util available (db2.js, db1.js)");
    throw e; // preserve original error to aid debugging
  }
}
...
//This function converts a time duration in minutes into a human-friendly string format.
/*
Example:
Input: n = 95
h = 1, m = 35
Output: "1h 35m (95.0 mins)" */
...
async function generateAttendanceUsagePDF({ day } = {}) {
  try {
    ...
    /*whereDay = "WHERE DATE(e.entry_time) = $1::date"; ($1-first value of the array)
      params = ["2025-10-24"];
      SELECT ... WHERE DATE(e.entry_time) = '2025-10-24'
 */
  ...
  // Peak entry times (hourly distribution)
    ...
    // Build WHERE/AND safely so it works with and without a day filter.
    const repeatOuterPrefix = filterDate ? `${whereDay} AND` : 'WHERE';
    const repeatInnerWhere = filterDate ? `WHERE DATE(entry_time) = $1::date` : '';
    const repeatVisitsDetailRes = await pool.query(`
      SELECT tag_id, building_id, entry_time
      FROM "EntryExitLog" e
      ${repeatOuterPrefix} tag_id IN (
        SELECT tag_id
        FROM "EntryExitLog"
        ${repeatInnerWhere}
        GROUP BY tag_id
    ...
    // Define time ranges used for per-slot analysis
    const slotRanges = [
    ...
    // Only show buildings present in totals (equivalent to metrics' buildings)
    const metricBuildingIds = (totalVisitsRes.rows || []).map(r => r.building_id);
    ...
    for (const slot of slotRanges) {
      ...
      // Filter to only buildings in totals/metrics
...
/*
{
  ...
  rowCount: 3,
  rows: [
    { dept_name: 'Engineering', building_id: 'BLD001', visits: 120 },
    { dept_name: 'Science', building_id: 'BLD002', visits: 98 },
    { dept_name: 'Arts', building_id: 'BLD003', visits: 75 }
  ]
}
...
/*
map() goes through each building in that array (each item is tv).
For each building, it finds matching data from other result sets 
(like unique visitors, average time, exits, etc.) and builds a combined record.
*/
  ...
  /*
  This combines all metrics for that building into one object.
  [
  {
    ...
    "Unique Visitors": "80",
    "Repeat Visits": "10",
    "Avg Duration": "45.5 mins",
    "Peak Entry Hour": "14:00 (230)",
    "Total Exits": "118"
  },
  {
    "Building": "Science",
    ...
    "Unique Visitors": "70",
    "Repeat Visits": "5",
    "Avg Duration": "39.2 mins",
    "Peak Entry Hour": "13:00 (180)",
    "Total Exits": "92"
  }
]
...
/*PDF is being made ->	attendance.pdf.part
PDF finished	Rename → attendance.pdf */
    const filepath = path.join(exportsDir, filename); 
    const tempPath = `${filepath}.part`;            

  // Create the PDF and pipe it into the temp file on disk
  ...
  // Finish writing the PDF content to the stream
  doc.end();

/* This waits for the file writing process to complete.
stream.on("finish") triggers when all data is written to disk.
stream.on("error") triggers if something goes wrong.
This verifies the code does not continue until the PDF is fully saved.
 */
    // Wait until the write stream finishes before proceeding
    ...
    try {
  //  Check if the temporary file (PDF still being written) actually exists and has content
  const stats = fs.statSync(tempPath);

  //  If file does not exist OR file size is 0 bytes → PDF export failed
  if (!stats || stats.size === 0) {
    throw new Error(`Export appears empty: ${tempPath}`);
  }

  //  "Atomic rename" → If PDF is complete, rename .part file to final .pdf file
  // This verifies that users never see a half-written or corrupted file.
  fs.renameSync(tempPath, filepath);

} catch (e) {
  //  If anything went wrong (file not found, rename failed, empty file):
  // Return a clear error to help with debugging
  throw new Error(`Export not saved correctly at ${tempPath}: ${e.message}`);
}

//  Export was successful → return the final path of the finished PDF
return filepath;

} catch (err) {
  //  If any unexpected error occurs during the entire generation:
