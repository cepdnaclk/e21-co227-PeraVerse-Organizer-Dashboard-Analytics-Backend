...
// Use local database for Event Summary
...
// Format minutes into a friendly "Hh Mm" string (e.g., 99.84 -> "1h 40m")
function fmtHoursMinutes(mins) {
  const n = safeNum(mins);
  if (!Number.isFinite(n) || n <= 0) return "0m";
  const h = Math.floor(n / 60);
  const m = Math.round(n % 60); // nearest minute
  if (h <= 0) return `${m}m`;
  return `${h}h ${m}m`;
}
...
async function generateEventPDF({ day } = {}) {
  try {
    ...
    drawKPI(
      ...
      startY,
      "Avg Duration",
      fmtHoursMinutes(summary.avg_duration)
    );
    drawKPI(
